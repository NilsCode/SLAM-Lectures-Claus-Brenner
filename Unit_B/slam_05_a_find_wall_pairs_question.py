...
def write_cylinders(file_desc, line_header, cylinder_list):
    ...
    # Write to file.
    out_string = line_header
    #print("D C",file = out_file)
    for c in cylinder_list:
        #print("%.1f %.1f" % c,file = out_file)
        out_string += " %.1f %.1f " % tuple(c)
    print(out_string,file =  file_desc)
    ...
    for point in points:
        ...
        if abs(x) <= arena_left + eps:
            ...
            if abs(y) <= arena_bottom + eps:
                right_list.append([arena_left, arena_bottom])
            
            elif abs(y) >= arena_top - eps:
                right_list.append([arena_left, arena_top])
            ...
            if abs(y) <= arena_bottom + eps:
                right_list.append([arena_right, arena_bottom])
            
            elif abs(y) >= arena_top - eps:
                right_list.append([arena_right, arena_top])
