...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        new_G = eye(3 + 2*self.number_of_landmarks)
        new_G[0:3, 0:3] = G3
        G3 = new_G
        
        new_V = zeros([3 + 2*self.number_of_landmarks, 2])
        new_V[0:3, 0:2] = V
        V = new_V
        R3 = dot(V, dot(control_covariance, V.T))
        self.covariance = dot(G3, dot(self.covariance, G3.T)) + R3  # Replace this.
        # state' = g(state, control)
        self.state[0:3] = self.g(self.state[0:3], control, self.robot_width)  # Replace this.
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        new_state = state_list + [ele for ele in initial_coords]
        index = int((len(state_list) - 3)/2)
        self.number_of_landmarks += 1
        self.state = array(new_state)
        
        new_cov = zeros([len(state_list) + 2, len(state_list) + 2])
        new_cov[-2,-2] = 500.0**2  # 300 mm in x.
        new_cov[-1,-1] = 500.0**2  # 500 mm in y.
        new_cov[0:len(state_list), 0:len(state_list)] = self.covariance
    ...
    def correct(self, measurement, landmark_index):
        ...
        new_H = zeros([2,3 + self.number_of_landmarks * 2])
        new_H[0:2, 0:3] = H3
        new_H[0:2, 3+2*landmark_index : 3+2*landmark_index+2] = -H3[0:2,0:2]
