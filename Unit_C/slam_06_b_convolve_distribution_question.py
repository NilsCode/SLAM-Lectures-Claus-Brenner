...
def convolve(a, b):
    ...
    for j,value in enumerate(b.values):
        noffset = b.offset + j - int((len(a.values) - 1)/2)
        new_values = [value * a_value for a_value in a.values]
        new_dist = Distribution(noffset, new_values)
        dist_list.append(new_dist)
    
    a = Distribution.sum(dist_list)
