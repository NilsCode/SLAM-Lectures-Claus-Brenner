...
def histogram_plot(prediction, measurement, correction):
    ...
    plot(prediction.plotlists(*arena)[0], prediction.plotlists(*arena)[1],
         color='#C0C0FF', linestyle='-',drawstyle = 'steps', linewidth=5)
    plot(measurement.plotlists(*arena)[0], measurement.plotlists(*arena)[1],
         color='#C0FFC0', linestyle='-',drawstyle = 'steps', linewidth=5)    
    plot(correction.plotlists(*arena)[0], correction.plotlists(*arena)[1],
         color='#FFC0C0', linestyle='-',drawstyle = 'steps', linewidth=5)
...
def kalman_filter_step(belief, control, measurement):
    ...
    # Prediction.
    mu_t_bar = belief.mu + control.mu
    sigma2_t_bar = belief.sigma2 + control.sigma2
    prediction = Density(mu_t_bar, sigma2_t_bar)  # Replace

    # Correction.
    K = sigma2_t_bar/(sigma2_t_bar + measurement.sigma2)
    mu = mu_t_bar + K * (measurement.mu - mu_t_bar)
    sigma2 = (1 - K) * sigma2_t_bar
